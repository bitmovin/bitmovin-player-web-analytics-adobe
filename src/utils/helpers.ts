import { Teardown, TeardownPlayerProjectionTuple, EventDataObj, PlayerWithItemProjection } from '../types/analytics';
...
import { PlayerAPI, PlayerEventCallback, StreamType, PlayerEvent } from 'bitmovin-player';
...
export const hasVideoPlaybackQuality = typeof document.createElement('video').getVideoPlaybackQuality === 'function';

const removeAtIndex = (arr: any[], i: number): any[] => [...arr.slice(0, i), ...arr.slice(i + 1, arr.length)];
...
export const hasPostAd = (player: PlayerAPI): Boolean => {
  ...
  if (scheduledAdBreakList.length > 0) {
    const positions = Object.keys(scheduledAdBreakList).map(x => scheduledAdBreakList[x].position);
...
export const toGetStreamType = (player: PlayerAPI): string => player.getStreamType();
 ...
 */
export const teardownAndRemove = (teardownArr: Teardown[], index: number): Teardown[] => {
  teardownArr[index]();
...
export const toEventDataObj = (eventType: PlayerEvent, callback: PlayerEventCallback): EventDataObj => ({
  eventType,
  callback,
});
 ...
 */
export const toGetValue = <T>(evtTpl: TeardownPlayerProjectionTuple<T>) => evtTpl[1];
/**
 * Accesses the teardown function from a TeardownPlayerProjectionTuple
 */
export const toTeardown = <T>(evtTpl: TeardownPlayerProjectionTuple<T>): Teardown => evtTpl[0];
